...
func _ready():
	...
	_animation.then({ node = $VBoxContainer/Button1, animation = "flash", duration = 1, on_completed = [callable(self, '_on_button_completed'), [1]] })
	_animation.then({ node = $VBoxContainer/Button2, animation = "tada", duration = 1, on_completed = [callable(self, '_on_button_completed'), [2]] })
	_animation.then({ node = $VBoxContainer/Button3, animation = "shakeX", duration = 1, on_completed = [callable(self, '_on_button_completed'), [3]] })
