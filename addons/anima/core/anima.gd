...
static func begin(node: Node, name: String = 'anima', single_shot := false):
	...
	for child in node.get_children():
		if String(child.name).find(node_name) >= 0 and is_instance_valid(child):
			anima_node = child
			anima_node.clear()
			anima_node.stop()
...
static func Group(group: Node, items_delay: float, animation_type: int = GROUP.FROM_TOP, point := 0) -> AnimaDeclarationGrid:
	var c := AnimaDeclarationGrid.new()

	return c._init_me(group, Vector2(0, group.get_child_count()), items_delay, animation_type, point)
